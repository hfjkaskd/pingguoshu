...
namespace AdvancedInputFieldPlugin
{
	public class InputFieldEngine
	{
		...
		internal void ProcessDone()
		{
			...
			// }
			if (InputField.NextInputField != null && !InputField.NextInputField.ReadOnly)
			{
				//Use click-like switching: select next first and let current field deselect naturally.
				endEditReason = EndEditReason.KEYBOARD_NEXT;
				InputField.NextInputField.ManualSelect(BeginEditReason.KEYBOARD_NEXT);
			}
			else
			{
		...
		internal IEnumerator DelayedDeselect()
		{
			...
			// ✅ 关键：如果正在切换到另一个输入框，放行，不要抢回
			if (IsOtherInputFieldSelected())
			{
				EndEditMode();
				DisableSelection();
				endEditReason = EndEditReason.USER_DESELECT;
				yield break;
			}

			if (IsCanvasFrontRendererSelected() || IsInputFieldChildSelected() || IsInputFieldRefocusableSelected())
			{
				Reselect();
			}
			else
			{
				if (IsAllowKeyboardDismissSelected())
				{
					EndEditMode();
					DisableSelection();
				}
		...
		internal bool IsAllowKeyboardDismissSelected()
		{
			GameObject currentSelection = EventSystem.current.currentSelectedGameObject;
			if (currentSelection == null) return false;
			return currentSelection.GetComponentInParent<AllowKeyboardDismiss>() != null;
		}

		internal bool IsOtherInputFieldSelected()
		{
			GameObject currentSelection = EventSystem.current?.currentSelectedGameObject;
			if (currentSelection == null) return false;

			AdvancedInputField other = currentSelection.GetComponentInParent<AdvancedInputField>();
			return other != null && other != InputField;
		}
