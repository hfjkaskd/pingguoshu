    ...
    public partial class Path
    {
        ...
        protected virtual void DrawLocalGizmos(ref Matrix4x4 matrix)
        {
            ...
            using (HandlesColorScope.New(_arrowColor))
            {
                ...
                HandlesUtils.DrawAALine(point, point + rotation * vector);
                vector.x = -vector.x;
                HandlesUtils.DrawAALine(point, point + rotation * vector);
            }
        }
        ...
        protected class Editor : BaseEditor<Path>
        {
            public override void OnInspectorGUI()
            {
                ...
                {
                    bool edit = GUI.Toggle(rect, FloatingWindow.target == target, EditorGUIUtility.IconContent("EditCollider"), EditorGUIUtils.buttonStyle);
                    rect.x = rect.xMax + 5;
                    rect.width = 140;
                    EditorGUI.LabelField(rect, "Edit Path", EditorGUIUtils.middleLeftLabelStyle);
                ...
                using (var scope = ChangeCheckScope.New(target))
                {
                    float value = EditorGUILayout.FloatField(
                        EditorGUIUtils.TempContent("World Scale", null, "Use \"World Scale\" to scale the path instead of scales of transform."),
                        target.worldScale);
                ...
                using (var scope = ChangeCheckScope.New(target))
                {
                    bool value = EditorGUIUtils.IndentedToggleButton("Always Visible", target._alwaysVisible);
        ...
        protected abstract class FloatingWindow : EditorWindow
        {
            ...
            void OnGUI()
            {
                ...
                if (path && path.floatingWindowType == GetType())
                {
                    ...
                    using (GUIContentColorScope.New(EditorGUIUtils.labelNormalColor))
                    {
                        var rect = EditorGUILayout.GetControlRect(false, 25);

                        rect.width /= 3;
                        if (GUI.Toggle(rect, selectedTool == 0,
                            EditorGUIUtils.TempContent(null, EditorResources.instance.moveToolPan, "Pan Move Tool"),
                            EditorGUIUtils.buttonLeftStyle))
                            selectedTool = 0;

                        rect.x = rect.xMax;
                        if (GUI.Toggle(rect, selectedTool == 1,
                            EditorGUIUtils.TempContent(null, EditorResources.instance.moveTool3D, "3D Move Tool"),
                            EditorGUIUtils.buttonMiddleStyle))
                            selectedTool = 1;
                        ...
                        using (DisabledScope.New(disableRotateTool))
                        {
                            ...
                            if (GUI.Toggle(rect, selectedTool == 2,
                                EditorGUIUtils.TempContent(null, EditorResources.instance.RotateTool, "Rotate Tool"),
                                EditorGUIUtils.buttonRightStyle))
                                selectedTool = 2;
                        }
                        ...
                        rect.width /= 3;
                        if (GUI.Button(rect,
                            EditorGUIUtils.TempContent(null, EditorResources.instance.addNodeBack, "Add Node Back"),
                            EditorGUIUtils.buttonLeftStyle))
                        {
                        ...
                        using (DisabledScope.New(path.nodeCount == 2))
                        {
                            rect.x = rect.xMax;
                            if (GUI.Button(rect,
                                EditorGUIUtils.TempContent(null, EditorResources.instance.removeNode, "Remove Node"),
                                EditorGUIUtils.buttonMiddleStyle))
                            {
                        ...
                        rect.x = rect.xMax;
                        if (GUI.Button(rect,
                            EditorGUIUtils.TempContent(null, EditorResources.instance.addNodeForward, "Add Node Forward"),
                            EditorGUIUtils.buttonRightStyle))
                        {
