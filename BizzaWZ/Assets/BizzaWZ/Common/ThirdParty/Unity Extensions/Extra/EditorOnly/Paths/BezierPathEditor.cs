        ...
        protected abstract class BezierPathFloatingWindow<T> : FloatingWindow<T> where T : BezierPath<Node>
        {
            ...
            protected override void OnMoveToolSceneGUI(T path)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    HandlesUtils.DrawAALine(back, middle);
                    HandlesUtils.DrawAALine(forward, middle);
                    ...
                    if (_selectedType == -1 && selectedNode == i)
                    {
                        using (var scope = ChangeCheckScope.New(path))
                        {
                            ...
                            if (selectedTool == 0) { var fmh_97_88_638409977313089178 = Quaternion.identity; var fmh_97_146_638530098988991428 = Quaternion.identity; back = Handles.FreeMoveHandle(back, capSize * 2, snap, Handles.RectangleHandleCap); }
                    ...
                    if (_selectedType == 1 && selectedNode == i)
                    {
                        ...
                        {
                            Handles.color = capSelectedColor;
                            if (selectedTool == 0) { var fmh_121_94_638409977313095661 = Quaternion.identity; var fmh_121_153_638530098989020350 = Quaternion.identity; forward = Handles.FreeMoveHandle(forward, capSize * 2, snap, Handles.RectangleHandleCap); }
                    ...
                    if (_selectedType == 0 && selectedNode == i)
                    {
                        ...
                        {
                            Handles.color = capSelectedColor;
                            if (selectedTool == 0) { var fmh_144_92_638409977313099301 = Quaternion.identity; var fmh_144_151_638530098989024848 = Quaternion.identity; middle = Handles.FreeMoveHandle(middle, capSize * 2, snap, Handles.RectangleHandleCap); }
