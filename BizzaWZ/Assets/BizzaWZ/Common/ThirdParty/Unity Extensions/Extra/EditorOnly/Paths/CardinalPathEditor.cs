            ...
            protected override void OnMoveToolSceneGUI(T path)
            {
                if (_showTension)
                {
                    for (int i = 0; i < path.segmentCount; i++)
                    {
                        ...
                        HandlesUtils.DrawAALine(point1, point2, 4);
                        ...
                        using (var scope = ChangeCheckScope.New(path))
                        {
                            ...
                            var fmh_82_67_638409977311670233 = Quaternion.identity; var fmh_82_123_638530098989254956 = Quaternion.identity; var fmh_82_180_638729793696141934 = Quaternion.identity; point = Handles.FreeMoveHandle(point, capSize, snap, Handles.CircleHandleCap);

                            tension = MathUtils.ClosestPointOnSegmentFactor(point, point1, point2);
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (selectedNode == i)
                    {
                        using (var scope = ChangeCheckScope.New(path))
                        {
                            ...
                            if (selectedTool == 0) { var fmh_118_96_638409977311699237 = Quaternion.identity; var fmh_118_155_638530098989258711 = Quaternion.identity; var fmh_118_213_638729793696164132 = Quaternion.identity; position = Handles.FreeMoveHandle(position, capSize * 2, snap, Handles.RectangleHandleCap); }
