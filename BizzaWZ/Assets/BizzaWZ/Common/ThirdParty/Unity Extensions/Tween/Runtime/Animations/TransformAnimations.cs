...
namespace UnityExtensions.Tween
{
    ...
    public class TweenTransformPosition : TweenVector3<Transform>
    {
        ...
        public override Vector3 GetCurrentState(Transform target)
        {
            return target ? (space == Space.World ? target.position : target.localPosition) : default;
        }

        public override void SetCurrentStateRaw(Transform target, Vector3 value)
        {
            if (target)
            {
    ...
    public class TweenTransformEulerAngles : TweenVector3<Transform>
    {
        ...
        public override Vector3 GetCurrentState(Transform target)
        {
            return target ? (space == Space.World ? target.eulerAngles : target.localEulerAngles) : default;
        }
    ...
    public class TweenTransformRotation : TweenQuaternion<Transform>
    {
        ...
        public override Quaternion GetCurrentState(Transform target)
        {
            return target ? (space == Space.World ? target.rotation : target.localRotation) : Quaternion.identity;
        }

        public override void SetCurrentState(Transform target, Quaternion value)
        {
            if (target)
            {
    ...
    public class TweenTransformScale : TweenVector3<Transform>
    {
        public override Vector3 GetCurrentState(Transform target)
        {
            return target ? target.localScale : Vector3.one;
        }
    ...
    public class TweenTransform : TweenFromTo<Transform>
    {
        ...
        protected override void OnPropertiesGUI(TweenPlayer player, SerializedProperty property)
        {
            ...
            rect.width = rect.height + EditorStyles.label.CalcSize(EditorGUIUtils.TempContent("P")).x;
            togglePositionProp.boolValue = EditorGUI.ToggleLeft(rect, "P", togglePositionProp.boolValue);

            rect.x = rect.xMax + rect.height * 0.5f;
            rect.width = rect.height + EditorStyles.label.CalcSize(EditorGUIUtils.TempContent("R")).x;
            toggleRotationProp.boolValue = EditorGUI.ToggleLeft(rect, "R", toggleRotationProp.boolValue);

            rect.x = rect.xMax + rect.height * 0.5f;
            rect.width = rect.height + EditorStyles.label.CalcSize(EditorGUIUtils.TempContent("S")).x;
            ...
            using (DisabledScope.New(!togglePosition && !toggleRotation && !toggleLocalScale))
            {
                using (LabelWidthScope.New(12))
                {
                    ...
                    EditorGUI.ObjectField(fromRect, fromProp, EditorGUIUtils.TempContent("F"));
                    EditorGUI.ObjectField(toRect, toProp, EditorGUIUtils.TempContent("T"));
                }
            }
        }
        ...
        protected override void CreateOptionsMenu(GenericMenu menu, TweenPlayer player, int index)
        {
            ...
            if (!from || !target) menu.AddDisabledItem("From = Current");
            ...
            if (!to || !target) menu.AddDisabledItem("To = Current");
            ...
            if (!from || !target) menu.AddDisabledItem("Current = From");
            ...
            if (!to || !target) menu.AddDisabledItem("Current = To");
