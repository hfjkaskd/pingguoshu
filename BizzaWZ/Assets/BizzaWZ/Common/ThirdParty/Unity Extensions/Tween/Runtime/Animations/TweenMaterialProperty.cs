    ...
    public class TweenMaterialProperty : TweenFromTo<Vector4>
    {
        ...
        public override void Interpolate(float factor)
        {
            if (propertyID != -1 && target)
            {
                ...
                using (MaterialPropertyBlockPool.global.Spawn(out var properties))
                {
                    ...
                    void SetPropertyBlockValue()
                    {
                        switch (_propertyType)
                        {
                            case Type.Color:
                            case Type.Vector:
                                properties.SetVector(propertyID, value);
                                return;

                            case Type.Float:
                            case Type.Range:
                                properties.SetFloat(propertyID, value.x);
                                return;
                        }
                    }
                }
        ...
        public override void RecordState()
        {
            ...
            if (target)
            {
                ...
                for (int i = 0; i < materialCount; i++)
                {
                    _tempBlocks.Add(MaterialPropertyBlockPool.global.Spawn());
        ...
        public override void RestoreState()
        {
            if (_originalTarget)
            {
                ...
                {
                    MaterialPropertyBlockPool.global.Despawn(item);
                }
                _tempBlocks.Clear();
            }
        }
        ...
        void DrawMaterialMask(int materialCount, TweenPlayer player)
        {
            ...
            rect = EditorGUI.PrefixLabel(rect, EditorGUIUtils.TempContent("Materials"));
        ...
        {
            var rect = EditorGUILayout.GetControlRect();
            rect = EditorGUI.PrefixLabel(rect, EditorGUIUtils.TempContent("Property"));
        ...
        protected override void OnPropertiesGUI(TweenPlayer player, SerializedProperty property)
        {
            ...
            switch (propertyType)
            {
                ...
                case Type.Color:
                    ...
                    {
                        fromProp.vector4Value = EditorGUI.ColorField(fromRect, EditorGUIUtils.TempContent("F"), fromProp.vector4Value, false, true, true);
                        toProp.vector4Value = EditorGUI.ColorField(toRect, EditorGUIUtils.TempContent("T"), toProp.vector4Value, false, true, true);
                    }
                    break;
            }
        }
