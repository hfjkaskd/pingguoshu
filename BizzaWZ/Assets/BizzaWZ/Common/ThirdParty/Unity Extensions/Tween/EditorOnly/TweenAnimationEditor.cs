...
namespace UnityExtensions.Tween
{
    public abstract partial class TweenAnimation
    {
        ...
        public virtual void Reset(TweenPlayer player)
        {
            ...
            _holdAfterEnd = true;
            _interpolator = new Interpolator(Interpolator.Type.Linear);
            _comment = null;
        }
        ...
        protected static void FromToFieldLayout(string label, ref float from, ref float to, out bool fromChanged, out bool toChanged)
        {
            ...
            var content = EditorGUIUtils.TempContent(label);
            EditorGUI.LabelField(rect, content);

            rect.width = EditorStyles.label.CalcSize(content).x;
            float delta = EditorGUIUtils.DragValue(rect, 0, 0.01f);
            ...
            using (LabelWidthScope.New(12))
            {
                ...
                if (delta != 0)
                {
                    ...
                    newFrom = MathUtils.RoundToSignificantDigitsFloat(newFrom + delta, 6);
                    newTo = MathUtils.RoundToSignificantDigitsFloat(newTo + delta, 6);
                }
        ...
        internal void OnInspectorGUI(int index, TweenPlayer player, SerializedProperty property)
        {
            ...
            property.isExpanded = GUI.Toggle(rect2, property.isExpanded, GUIContent.none, EditorStyles.foldout);
            ...
            var optionsIcon = EditorGUIUtils.paneOptionsIcon;
            ...
            // foldout
            property.isExpanded = GUI.Toggle(rect2, property.isExpanded, allTypes[GetType()].name, EditorStyles.boldLabel);
            ...
            if (GUI.Button(rect, EditorGUIUtils.TempContent(image: optionsIcon), GUIStyle.none))
            {
            ...
            if (property.isExpanded)
            {
                using (var scope = ChangeCheckScope.New(player))
                {
                    float min = minNormalizedTime;
                    float max = maxNormalizedTime;
