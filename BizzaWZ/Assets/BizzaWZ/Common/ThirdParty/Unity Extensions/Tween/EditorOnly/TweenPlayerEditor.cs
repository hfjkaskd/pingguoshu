...
using EditorResources = UnityExtensions.Tween.Editor.EditorResources;
...
namespace UnityExtensions.Tween
{
    public partial class TweenPlayer
    {
        ...
        bool _enabledRecord;
        public float _normalizedTimeRecord;
            ...
            set
            {
                ...
                else
                {
                    if (_preview != value)
                    {
                        ...
                        if (value)
                        {
                            UnityExtensions.Editor.EditorUtils.editModeUpdate += OnUpdate;
        ...
        internal class Editor : BaseEditor<TweenPlayer>
        {
            ...
            internal static Color separatorLineColor
            {
                get
                {
                    var lineColor = EditorGUIUtils.labelNormalColor;
                    lineColor.a *= 0.25f;
                    return lineColor;
                }
            }
            ...
            void ShowAddMenu(Rect rect)
            {
                if (_addMenu == null)
                {
                    _addMenu = EditorGUIUtils.CreateMenu(
            ...
            void OnEnable()
            {
                ...
                _syncOnAwakeProp = serializedObject.FindProperty(nameof(sampleOnEnable));
            ...
            {
                using (GUIColorScope.New(target.playing ? new Color(0.85f, 1f, 0.85f) : Color.white))
                {
                    ...
                    if (!serializedObject.isEditingMultipleObjects)
                    {
                        ...
                        using (GUIContentColorScope.New(target.playing ? progressForegroundValid : EditorGUIUtils.labelNormalColor))
                        {
                            target.playing = GUI.Toggle(rect2, target.playing,
                                EditorGUIUtils.TempContent(image: EditorResources.instance.play), imageButtonStyle);
                        }
                        ...
                        using (DisabledScope.New(!target.playing))
                        {
                            ...
                            {
                                if (GUI.Button(rect2, EditorGUIUtils.TempContent(image: target.direction == PlayDirection.Forward ?
                        ...
                        {
                            float progress = EditorGUIUtils.DragProgress(rect, target._normalizedTime, progressBackgroundValid, progressForegroundValid);
