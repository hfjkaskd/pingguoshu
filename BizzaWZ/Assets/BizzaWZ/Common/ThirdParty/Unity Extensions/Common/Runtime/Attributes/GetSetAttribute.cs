...
namespace UnityExtensions
{
    ...
    public sealed class GetSetAttribute : PropertyAttribute
    {
        ...
        string _label;
        GetSetMode _mode;
        PropertyInfo _propertyContainer;
        ...
        class GetSetDrawer : BasePropertyDrawer<GetSetAttribute>
        {
            ...
            public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
            {
                ...
                {
                    attribute._propertyContainer = fieldInfo.DeclaringType.GetInstanceProperty(attribute._propertyName);
                    attribute._propertyName = null;
                }

                if (attribute._propertyContainer == null || !attribute._propertyContainer.CanRead)
                {
                    return EditorGUIUtility.singleLineHeight;
                }

                var parentObject = property.GetParentValue();
                var propertyValue = attribute._propertyContainer.GetValue(parentObject, null);
            ...
            public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
            {
                ...
                if (attribute._propertyContainer == null)
                {
                    EditorGUI.LabelField(position, label.text, "Can't find the property");
                    return;
                }

                if (!attribute._propertyContainer.CanRead)
                {
                ...
                using (DisabledScope.New(!attribute._propertyContainer.CanWrite))
                {
                    using (var scope = ChangeCheckScope.New())
                    {
                        ...
                        if (scope.changed)
                        {
                            attribute._propertyContainer.SetValue(parentObject, propertyValue, null);
                            property.SetParentValue(parentObject);

                            foreach (var target in property.serializedObject.targetObjects)
                            {
                                if (target != property.serializedObject.targetObject)
                                {
                                    parentObject = property.GetParentValue(target);
                                    attribute._propertyContainer.SetValue(parentObject, propertyValue, null);
                                    property.SetParentValue(target, parentObject);
                                }
                            }
            ...
            float GetBuiltInHeight(object propertyValue, bool isExpanded)
            {
                ...
                return EditorGUIUtils.GetMultipleLinesHeight(lines);
            }
            ...
            bool OnBuiltInGUI(ref object propertyValue, Rect position, bool isExpanded, GUIContent label, UnityObject[] objects)
            {
                ...
                {
                    if (typeof(UnityObject).IsAssignableFrom(attribute._propertyContainer.PropertyType))
                    {
                        ...
                        value = EditorGUI.ObjectField(position, label, value, attribute._propertyContainer.PropertyType, allowScene);
                    ...
                    else
                    {
                        switch (propertyValue)
                        {
                            ...
                            case Vector2 value:
                                {
                                    value = EditorGUIUtils.SingleLineVector2Field(position, label, value);
                                    if (scope.changed) propertyValue = value;
                                    break;
                                }
                            ...
                            case Vector2Int value:
                                {
                                    value = EditorGUIUtils.SingleLineVector2IntField(position, label, value);
                                    if (scope.changed) propertyValue = value;
                                    break;
                                }
                            ...
                            case Vector4 value:
                                {
                                    value = EditorGUIUtils.TwoLinesVector4Field(position, label, value);
                                    if (scope.changed) propertyValue = value;
                                    break;
                                }
                            ...
                            case Vector4Int value:
                                {
                                    value = EditorGUIUtils.TwoLinesVector4IntField(position, label, value);
                                    if (scope.changed) propertyValue = value;
                                    break;
                                }
                            ...
                            case Rect value:
                                {
                                    value = EditorGUIUtils.TwoLinesRectField(position, label, value);
                                    if (scope.changed) propertyValue = value;
                                    break;
                                }
                            ...
                            case RectInt value:
                                {
                                    value = EditorGUIUtils.TwoLinesRectIntField(position, label, value);
                                    if (scope.changed) propertyValue = value;
                                    break;
                                }
